...
class LoadEsewa:
    ...
    def __init__(self, root, user_id):
        ...
        self.InfoLabel = Label(self.frame1,text='Receipent Phone Number',font='Nunito 18')
        ...
        self.send_btn = Button(self.frame1, font='Nunito 16', text='Send',width=30, command=self.load_esewa)
        self.send_btn.grid(row=8,columnspan=4, padx=10, pady=10)
    ...
    def load_esewa(self):
        if  self.recept_acc_id.get() == '' or self.amount_entry.get() == '' or self.invoice_entry.get("1.0",'end-1c') == '':
            ...
            return False
        response = Transaction(self.user_id).load_esewa(self.account_datas.get(), self.amount_entry.get())
        if response:
